...
def transfer_funds_using_test_data_key(test_context: FrameworkContext, test_data_key: str) -> None:
    ...
    CommonAssertions.assert_not_none(
        home_page,
        "Home page should be available before selecting transfer candidate accounts.",
    )
    ...
    source_account, destination_account = accounts_overview_page.get_transfer_candidate_accounts(transfer_amount_text)
...
def verify_transfer_completed_successfully(test_context: FrameworkContext) -> None:
    ...
    CommonAssertions.assert_not_none(
        source_account,
        "Source account should be available in scenario context for transfer validation.",
    )
    CommonAssertions.assert_not_none(
        destination_account,
        "Destination account should be available in scenario context for transfer validation.",
    )
    ...
    CommonAssertions.assert_equals(
        transfer_funds_page.get_result_from_account_value().strip(),
        str(source_account),
        "Transfer result page source account is incorrect.",
    )
    CommonAssertions.assert_equals(
        transfer_funds_page.get_result_to_account_value().strip(),
        str(destination_account),
        "Transfer result page destination account is incorrect.",
    )
